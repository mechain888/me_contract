...
contract MEYield is Context, ReentrancyGuard,Ownable {
    ...
    function Get7daysPerProduction() public view returns (uint256){
        if (GetTotalShare() == 0) {
            return 0;
        } else {
            return (GetRewardPerBlock().mul(Get7DaysShare()).div(GetTotalShare()));
        }
    }
    
    function Get30daysPerProduction() public view returns (uint256){
        if (GetTotalShare() == 0) {
            return 0;
        } else {
            return (GetRewardPerBlock().mul(Get30DaysShare()).div(GetTotalShare()));
        }       
    }
    
    
    function Get90daysPerProduction() public view returns (uint256){
        if (GetTotalShare() == 0) {
            return 0;
        } else {
            return (GetRewardPerBlock().mul(Get90DaysShare()).div(GetTotalShare()));
        }            
    }
    function Get360daysPerProduction() public view returns (uint256){
        if (GetTotalShare() == 0) {
            return 0;
        } else {
            return (GetRewardPerBlock().mul(Get360DaysShare()).div(GetTotalShare()));
        }             
    }
